...
function upsertAgentDirectoryReplica(
...
): { agentId: string; stoppedOngoing: boolean } {
  ...
  return {
    ...
    stoppedOngoing:
      ...
      isAgentOngoing({
        status: previousAgent.status,
        goalStatus: previousAgent.goal?.status,
      }) &&
      !isAgentOngoing({
