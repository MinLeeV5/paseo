...
describe("replaceFetchedAgentDirectory", () => {
  ...
  it("keeps newer metadata while accepting usage-only updates and legacy workspace ownership", () => {
    ...
    expect({
      ...
      stoppedOngoing: staleResult.stoppedOngoing,
  ...
  it("does not stop queue ownership during an active-goal idle gap", () => {
    const serverId = "server-goal-transition";
    const agentId = "goal-agent";
    const store = useSessionStore.getState();
    store.initializeSession(serverId, null as unknown as DaemonClient);
    const activeGoal = { objective: "Ship Goal support", status: "active" as const };
    ...
    const idleGap = createAgentPayload({
      ...initial,
      updatedAt: "2026-07-12T11:00:00.000Z",
    });
    const gapResult = applyAgentDirectoryDelta({
      serverId,
      delta: { kind: "upsert", agent: idleGap, project: createEntry(idleGap).project },
    });
    expect(gapResult.stoppedOngoing).toBe(false);

    const completed = createAgentPayload({
      ...idleGap,
      goal: { ...activeGoal, status: "complete" },
      updatedAt: "2026-07-12T12:00:00.000Z",
    });
    const completedResult = applyAgentDirectoryDelta({
      serverId,
      delta: { kind: "upsert", agent: completed, project: createEntry(completed).project },
    });
    expect(completedResult.stoppedOngoing).toBe(true);
