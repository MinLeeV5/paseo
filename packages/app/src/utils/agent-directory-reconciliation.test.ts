...
describe("agent directory reconciliation", () => {
  it("reports snapshot-only and buffered running transitions exactly once", () => {
    ...
    expect(result.stoppedOngoingAgentIds).toEqual(["snapshot", "buffered"]);
  ...
  it("waits through an active-goal idle gap before reporting completion", () => {
    const activeGoal = { objective: "Ship Goal support", status: "active" as const };
    const result = reconcileAgentDirectory({
      previous: new Map([["goal", { ...replica("goal", "idle"), goal: activeGoal }]]),
      snapshot: [
        {
          ...entry("goal", "idle"),
          agent: { ...snapshot("goal", "idle"), goal: activeGoal },
        },
      ],
      deltas: [
        {
          ...
          project: entry("goal", "idle").project,
        },
      ],
    });

    expect(result.stoppedOngoingAgentIds).toEqual(["goal"]);
    expect(result.entries[0]?.agent.goal?.status).toBe("complete");
  });
  ...
  it("keeps newer page metadata when a stale buffered upsert arrives", () => {
    ...
    expect({
      ...
      stopped: result.stoppedOngoingAgentIds,
  ...
  it("clears a snapshot stop when a newer buffered upsert is running", () => {
    ...
    expect(result.stoppedOngoingAgentIds).toEqual([]);
  });
