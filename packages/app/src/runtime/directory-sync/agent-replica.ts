...
export class AgentDirectoryReplica {
  ...
  constructor(
    ...
    private readonly onStoppedOngoing: (agentId: string) => void,
  ) {}
  ...
  applyDelta(delta: AgentDirectoryDelta): void {
    ...
    if (result.stoppedOngoing) this.onStoppedOngoing(result.agentId);
  }
  ...
  commitSnapshot(
    ...
    for (const agentId of reconciled.stoppedOngoingAgentIds) this.onStoppedOngoing(agentId);
    return agents;
  }
