...
| `labels`              | `Record<string, string>`                 | Key-value labels (default `{}`). `paseo.parent-agent-id` is set automatically for agent-scoped creation and removed by detach; `paseo.open-agent-tab.*` tracks client-scoped managed-subagent tabs — see [agent-lifecycle.md](./agent-lifecycle.md)                                                                                                                                 |
...
| `goal`                | `AgentGoal?`                             | Last provider-owned Goal projection (`objective` and `status`).                                                                                                                                                                                                                                                                                                                     |
| `archivedGoal`        | `ArchivedGoal?`                          | Presentation-only marker for the currently hidden Goal strip. It stores the archived objective and timestamp, and is cleared when the provider reports no Goal or a different objective.                                                                                                                                                                                            |
...
| `sessionDiffBaseline` | `SessionDiffBaseline?`                   | Git snapshot captured immediately before the agent's first turn. It anchors the Changes panel's agent-session diff and is preserved across later snapshot writes and daemon restarts.                                                                                                                                                                                               |
| `turnDiffRecords`     | `TurnDiffRecord[]?`                      | Chronological user-Prompt records with immutable start/end Git snapshots. Running records omit the end snapshot. Preserved by normal snapshots and archive; removed only with hard Agent deletion.                                                                                                                                                                                  |

### Nested: ArchivedGoal

| Field        | Type                | Description                                                            |
| ------------ | ------------------- | ---------------------------------------------------------------------- |
| `objective`  | `string`            | Objective used to bind the marker to one provider Goal                 |
| `archivedAt` | `string` (ISO 8601) | When the user archived the Goal strip; this does not archive the Agent |

### Nested: SessionDiffBaseline

| Field        | Type                | Description                                                                                                  |
| ------------ | ------------------- | ------------------------------------------------------------------------------------------------------------ |
| `ref`        | `string`            | Hidden repository ref, `refs/paseo/session-baselines/{agentId}`                                              |
| `commit`     | `string`            | Parentless snapshot commit containing the tracked and non-ignored working-tree state at session start        |
| `repoRoot`   | `string`            | Exact checkout/worktree root captured for the agent; never the main repository root inferred from a worktree |
| `capturedAt` | `string` (ISO 8601) | Capture timestamp                                                                                            |

This field remains the compatibility boundary for clients that omit a Prompt turn ID. New turn-scoped UI reads `turnDiffRecords` instead.

### Nested: TurnDiffRecord

| Field            | Type                                                 | Description                                                          |
| ---------------- | ---------------------------------------------------- | -------------------------------------------------------------------- |
| `id`             | `string`                                             | Paseo-generated record UUID used by subscriptions and hidden refs    |
| `messageId`      | `string?`                                            | Composer/user-message identity when the sending surface provides one |
| `providerTurnId` | `string?`                                            | Provider-native turn ID, attached after `startTurn` succeeds         |
| `prompt`         | `string`                                             | User Prompt text used by the Changes selector                        |
| `status`         | `"running" \| "completed" \| "failed" \| "canceled"` | Turn snapshot state                                                  |
| `startSnapshot`  | `SessionDiffBaseline`                                | Checkout snapshot taken immediately before the Prompt is sent        |
| `endSnapshot`    | `SessionDiffBaseline?`                               | Checkout snapshot frozen on the terminal event; absent while running |
| `startedAt`      | `string` (ISO 8601)                                  | Start capture timestamp                                              |
| `endedAt`        | `string?` (ISO 8601)                                 | Terminal capture timestamp                                           |

Turn snapshots use a temporary Git index under `$PASEO_HOME/session-diff-captures/`; they never stage files in the user's real index. Hidden refs live at `refs/paseo/turn-snapshots/{agentId}/{recordId}/{start|end}`. Existing agent records without the optional array begin tracking on their next user Prompt. Hard-deleting an Agent removes both per-Prompt refs and the legacy session ref; archiving preserves them because the conversation remains resumable.
