...
Codex skill discovery uses the app-server `skills/list` response and always merges a file-system fallback. Keep that fallback aligned with project/user skill roots (`.agents/skills`, `.codex/skills`, and repo `skills`) so slash-command autocomplete and `/skill-name` routing still work when app-server metadata is missing project entries, including symlinked `.agents/skills` directories. Codex does not expose markdown command discovery through app-server today; Paseo scans project/user `.agents/commands`, `.codex/commands`, and `CODEX_HOME/commands` directly and expands those files before sending the turn.
